...
# -*- coding: utf-8 -*-
"""
This file contains the policy implementations. 

Acknowledgement: This code is in very large parts copied 
from https://github.com/keras-rl/keras-rl by Matthias Plappert.
"""
import random
import sys
...
sys.path.append('..')
from util import get_object_config  # pylint: disable=no-name-in-module


def prepare_policy(params, private_params):
    """Prepares the replay memory according to the settings in the
    ...
    -------
    policy : Policy
        Policy as initialized with the parameter specifications.
    """
    if params['policy'] == 'epsilon-decay':
        inner_policy = EpsGreedyQPolicy(seed=private_params['seed'])
        if params['factor_decay'] is not None:
            decay_steps = int(params['factor_decay'] * params['total_steps'])
        else:
            decay_steps = params['factor_steps']
        return LinearAnnealedPolicy(inner_policy,
                                      attr=params['policy_factor'],
                                      value_max=params['factor_max'],
                                      value_min=params['factor_min'],
                                      value_test=params['factor_test'],
                                      nb_steps=decay_steps,
                                      seed=private_params['seed'])
    elif params['policy'] == 'boltzmann':
        return BoltzmannQPolicy(seed=private_params['seed'])
    else:
        print('Policy "{}" not implemented'.format(params['policy']))
        sys.exit()
...
class LinearAnnealedPolicy(Policy):
    ...
    def __init__(self, inner_policy, attr, value_max, value_min, value_test, nb_steps, seed=123):
        if not hasattr(inner_policy, attr):
            raise ValueError('Policy does not have attribute "{}".'.format(attr))

        super(LinearAnnealedPolicy, self).__init__(seed)
...
class EpsGreedyQPolicy(Policy):
    ...
    """
    def __init__(self, eps=.1, seed=123):
        super(EpsGreedyQPolicy, self).__init__(seed)
        self.eps = eps
...
class GreedyQPolicy(Policy):
    ...
    def __init__(self, seed=123):
        super(GreedyQPolicy, self).__init__(seed)
...
class SoftmaxPolicy(Policy):
    ...
    """
    def __init__(self, seed=123):
        super(SoftmaxPolicy, self).__init__(seed)
...
class BoltzmannQPolicy(Policy):
    ...
    """
    def __init__(self, tau=1., clip=(-500., 500.), seed=123):
        super(BoltzmannQPolicy, self).__init__(seed)
        self.tau = tau
        self.clip = clip
...
class MaxBoltzmannQPolicy(Policy):
    ...
    """
    def __init__(self, eps=.1, tau=1., clip=(-500., 500.), seed=123):
        super(MaxBoltzmannQPolicy, self).__init__(seed)
        self.eps = eps
        self.tau = tau
        self.clip = clip
...
class BoltzmannGumbelQPolicy(Policy):
    ...
    def __init__(self, C=1.0, seed=123):
        assert C > 0, "BoltzmannGumbelQPolicy C parameter must be > 0, not " + repr(C)
        super(BoltzmannGumbelQPolicy, self).__init__(seed)
        self.C = C
