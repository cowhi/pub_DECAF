...
# This constant determines how many eras a checkpoint is kept for.
#CHECKPOINT_DURATION = 4
#MAX_SAMPLE_ATTEMPTS = 1000


def prepare_memory(params, private_params):
    ...
    -------
    memory : Memory
        Memory as initialized with the parameter specifications.
    """
    return WrappedReplayBuffer(
      observation_shape=private_params['observation_shape'],
      stack_size=params['stack_size'],
      use_staging=params['use_staging'],
      replay_capacity=params['memory_size'],
      batch_size=params['batch_size'],
      update_horizon=params['update_horizon'],
      gamma=params['gamma'],
      wrapped_memory=params['wrapped_memory'],
      max_sample_attempts=params['max_sample_attempts'],
      max_checkpoints=params['checkpoints_keep_max'],
      extra_storage_types=params['extra_storage_types'],
      observation_dtype=private_params['observation_dtype'])
...
class OutOfGraphReplayBuffer(object):
  ...
  def save(self, checkpoint_dir, era_number):
    ...
    Args:
      checkpoint_dir: str, the directory where numpy checkpoint files should be
        saved.
      era_number: int, era_number to use as a suffix in naming
    ...
    for attr in checkpointable_elements:
      filename = self._generate_filename(checkpoint_dir, attr, era_number)
      ...
      # that are older than max_checkpoints.
      stale_era_number = era_number - self._max_checkpoints
...
class WrappedReplayBuffer(object):
  ...
  def save(self, checkpoint_dir, era_number):
    ...
    Args:
      checkpoint_dir: str, the directory where to read the numpy checkpointed
        files from.
      era_number: int, the era_number to use as a suffix in naming
        numpy checkpoint files.
    """
